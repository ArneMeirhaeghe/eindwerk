...
export default function useLiveSession(link: string) {
  ...
  // 1) Laad de publieke sessie + blueprint + bestaande antwoorden
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
  ...
  const saveField = useCallback(
    async (sectionId: string, componentId: string, value: any) => {
      await patchField(link, sectionId, componentId, value);
      setResponses((prev) => ({
        ...prev,
        [sectionId]: { ...(prev[sectionId] || {}), [componentId]: value },
      }));
    },
    [link]
  );

  // 3) Bulk save voor gehele sectie
  ...
  // 4) Upload bestand en update responses
  const uploadFile = useCallback(
    async (sectionId: string, componentId: string, file: File) => {
      ...
      setResponses((prev) => {
        const prevArr = (prev[sectionId]?.[componentId] as any[]) || [];
        return {
          ...prev,
          [sectionId]: {
            ...(prev[sectionId] || {}),
            [componentId]: [...prevArr, mediaItem],
          },
        };
      });
    },

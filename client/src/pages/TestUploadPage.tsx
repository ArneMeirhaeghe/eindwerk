...
const TestUploadPage: React.FC = () => {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (!file) return;
    try {
      await uploadFile(file, tab, alt, styles);
      setFile(null); setAlt(''); setStyles('');
      fetchAll();
      setError('');
    } catch (err: any) {
      ...
      <div className="w-1/2 p-6 border-r">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          <div>
            ...
            <input type="file"
              accept={tab==='image'?'image/*':tab==='video'?'video/*':'*/*'}
          ...
          <div>
            <label className="block mb-1">Styles (bv. "width:100px;")</label>
          ...
          {uploads.map(u => (
            <div key={u.id} className="border p-3 rounded">
              {u.contentType.startsWith('image/') && (
                <img src={u.url} alt={u.alt} style={parseStyles(u.styles)} />
              )}
              {u.contentType.startsWith('video/') && (
                <video controls src={u.url} style={parseStyles(u.styles)} />
              )}
              {!u.contentType.startsWith('image/') && !u.contentType.startsWith('video/') && (
                <a href={u.url} target="_blank" rel="noopener noreferrer"
                   style={parseStyles(u.styles)} className="text-blue-600 underline">
                  {u.filename}
                </a>
              )}
            </div>

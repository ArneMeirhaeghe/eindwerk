  ...
  useEffect(() => {
    ;(async () => {
      try {
        ...
        setSession(s)
        // initialize all fases open
        const fasesState: Record<string, boolean> = {}
        Object.keys(s.fases).forEach(f => { fasesState[f] = true })
        setOpenFases(fasesState)
        // Prefetch forms...
    ...
    <div className="container mx-auto px-4 py-6 space-y-12">
      ...
      {Object.entries(session.fases).map(([fase, secs]) => (
        <section key={fase} className="space-y-4">
          <button
            className="flex items-center text-2xl font-semibold text-blue-600 border-b-2 border-blue-200 pb-2"
            onClick={() => toggleFase(fase)}
          >
            {openFases[fase] ? <ChevronDown className="mr-2" /> : <ChevronRight className="mr-2" />}

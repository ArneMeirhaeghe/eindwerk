...
import {
  FaHome, FaClipboardList, FaMapMarkedAlt, FaBoxOpen, FaUpload,
  FaWpforms, FaUsers, FaBars, FaTimes
} from "react-icons/fa"
...
export default function Sidebar() {
  ...
  // desktop sidebar, hidden on builder page
  const Desktop = (
    <aside className="hidden lg:flex flex-col justify-between fixed top-0 left-0 h-full bg-white border-r shadow-sm
                       overflow-y-auto transition-all w-16 hover:w-64 group/sidebar z-40">
      <div className="p-4 border-b flex items-center justify-center group-hover/sidebar:justify-start">
        <Link to="/" className="flex items-center gap-2 text-blue-600">
          <FaHome className="text-xl" />
          <span className="hidden group-hover/sidebar:inline font-semibold">Home</span>
        </Link>
      </div>
      ...
      <nav className="p-2 space-y-1 flex-1">
        <NavLink to="/" className={({isActive})=>linkClass(isActive)}>
          ...
          <span className="hidden group-hover/sidebar:inline">Overzicht verhuur</span>
        </NavLink>
        ...
        <NavLink to="/tours" className={({isActive})=>linkClass(isActive)}>
          ...
          <span className="hidden group-hover/sidebar:inline">Rondleidingen</span>
        </NavLink>
        ...
        {tours.length > 3 && (
          <NavLink to="/tours" className="ml-6 block text-sm py-1 px-3 text-gray-500 hover:bg-gray-100 rounded hidden group-hover/sidebar:block">
            …
          </NavLink>
        )}
        ...
        <NavLink to="/upload-zone" className={({isActive})=>linkClass(isActive)}>
          <FaUpload className="text-lg"/>
          <span className="hidden group-hover/sidebar:inline">Bestanden</span>
        </NavLink>
        <NavLink to="/formbuilder" className={({isActive})=>linkClass(isActive)}>
        ...
        {isAdmin && (
          <NavLink to="/users" className={({isActive})=>linkClass(isActive)}>
            ...
            <span className="hidden group-hover/sidebar:inline">Gebruikers</span>
          </NavLink>
        )}
      </nav>
  ...
  // mobile/hamburger, always shown on builder page
  const Mobile = (
    <>
      <button
        ...
        <>
          ...
          <aside className="fixed inset-y-0 left-0 w-64 bg-white shadow-lg z-50 flex flex-col">
            <div className="flex justify-between items-center p-4 border-b">
              ...
              <button onClick={()=>setOpen(false)} className="text-gray-600"><FaTimes/></button>
            </div>
            <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
              ...
              <NavLink to="/tours" className={({isActive})=>linkClass(isActive)} onClick={()=>setOpen(false)}>
                <FaMapMarkedAlt/> Rondleidingen
              </NavLink>
              {tours.slice(0,3).map(t=>(
                <NavLink key={t.id} to={`/tours/${t.id}/builder`} className="ml-4 text-sm hover:bg-gray-100 rounded" onClick={()=>setOpen(false)}>
                  {t.naamLocatie}
                </NavLink>
              ))}
              {tours.length>3 && (
                <NavLink to="/tours" className="ml-4 text-sm text-gray-500 hover:bg-gray-100 rounded" onClick={()=>setOpen(false)}>
                  …
                </NavLink>
              )}
              <NavLink to="/inventory" className={({isActive})=>linkClass(isActive)} onClick={()=>setOpen(false)}>
                <FaBoxOpen/> Checklijst
              </NavLink>
              <NavLink to="/upload-zone" className={({isActive})=>linkClass(isActive)} onClick={()=>setOpen(false)}>
                <FaUpload/> Bestanden
              </NavLink>
              <NavLink to="/formbuilder" className={({isActive})=>linkClass(isActive)} onClick={()=>setOpen(false)}>
                <FaWpforms/> Formulieren
              </NavLink>
              {isAdmin && (
                <NavLink to="/users" className={({isActive})=>linkClass(isActive)} onClick={()=>setOpen(false)}>
                  <FaUsers/> Gebruikers
                </NavLink>
              )}
            </nav>

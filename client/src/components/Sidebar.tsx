...
export default function Sidebar() {
  ...
  const Desktop = (
    <aside className="hidden lg:flex flex-col justify-between fixed top-0 left-0 h-full bg-white border-r shadow-sm overflow-y-auto transition-all group/sidebar z-40
                       w-16 hover:w-64">
      ...
      <nav className="p-2 space-y-1 flex-1">
        <NavLink to="/verhuur" className={({ isActive }) => linkClass(isActive)}>
  ...
  const Mobile = (
    <>
      <button
        className="fixed top-4 left-4 z-50 lg:hidden p-2 bg-blue-600 text-white rounded shadow"
        ...
        <>
          ...
          <aside className="fixed inset-y-0 left-0 w-64 bg-white shadow-lg z-50 flex flex-col">
            ...
            <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
              <NavLink to="/verhuur" className={({ isActive }) => linkClass(isActive)} onClick={() => setOpen(false)}>
